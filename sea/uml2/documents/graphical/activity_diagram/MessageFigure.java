...
import documents.concepts.activity_diagram.Message;
import documents.graphical.deployment_diagram.ExtendedTextFigure;
import documents.util.I18NProperties;

public class MessageFigure extends ActivityFigure
{
     ...
     */
    private static final String TYPE = new String("<<message>>");
    ...
    public void redraw()
    {
        ...
        if ((message.name() == null) || ("").equals(message.name()))
        {
        ...
        int widthFig1 = metrics1.stringWidth(messageName);
    ...
    protected JPopupMenu createPopupMenu()
    {
        ...
        popupMenu.add(new AbstractAction(I18NProperties.getString("change.name.message.activity"))
        {
            ...
            public void actionPerformed(ActionEvent event)
            {
                if (SingleConnector.managerView().openDialogInterface(Constantes.NAME_SPEC,
                    ((Message) concept()).name().trim()))
                {
                    ((Message) concept()).name((SingleConnector.manager()).getCreationName());
