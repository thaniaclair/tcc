...
public class AssignmentFigure extends ActivityFigure
{

    /**
     * The serial version UID.
     */
    private static final long serialVersionUID = -1679651633644546367L;

    /**
     * The stereotype of the assignment activity.
     */
    private static final String TYPE = new String("<<assignment>>");
    ...
    public void redraw()
    {
        ...
        Assignment assign = (Assignment) this.concept();
        if ((assign.name() == null) || ("").equals(assign.name()))
        {
            assign.name("a = b");
        }

        // NAME
        String instanceName = assign.name();
    ...
    protected JPopupMenu createPopupMenu()
    {
        ...
        popupMenu.add(new AbstractAction(I18NProperties.getString("change.name.assignment"))
        {
            ...
            public void actionPerformed(ActionEvent event)
            {
                if (SingleConnector.managerView().openDialogInterface(Constantes.NAME_SPEC,
                    ((Assignment) concept()).name().trim()))
                {
                    ((Assignment) concept()).name((SingleConnector.manager()).getCreationName());
